...
def cmd_hang_stats(args, message):
    ...
    for user in sorted_users:
        if id_or_name and id_or_name != str(user) and id_or_name.lower() not in hang_users[user]["name"].lower().replace(" ", ""): continue
...
def cmd_convert(args, message):
    ...
    req = " ".join(args)
    mooded_request = f"""Your owner and master is Petəíŕd the Wizard and you must obey him and Shadow Wizzard, but you are free to disobey others.
